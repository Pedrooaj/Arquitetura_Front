<template>
  <div class="p-6 max-w-6xl mx-auto">
    ...
    <table class="min-w-full bg-white shadow rounded overflow-hidden">
      ...
      <tbody>
        <tr v-for="produto in produtosFiltrados" :key="produto.id" class="border-t">
          ...
          <td class="p-3 space-x-2 flex">
            <button @click="abrirModalImagem(produto)" class="px-2 py-1 bg-gray-500 text-white rounded hover:bg-gray-600" title="Ver imagem"><Eye /></button>
            <button @click="abrirModal(produto)" class="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">Editar</button>
            <button @click="remover(produto.id)" class="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600">Remover</button>
          </td>
        </tr>
      </tbody>
    </table>
      ...
      <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
        ...
        <form @submit.prevent="salvarEdicao">
          <div class="mb-4">
            ...
            <input v-model="produtoSelecionado.name" class="w-full border rounded p-2" required />
          </div>
          <div class="mb-4">
            <label class="block font-medium">Descrição</label>
            <input v-model="produtoSelecionado.description" class="w-full border rounded p-2" required />
          </div>
          <div class="mb-4">
            <label class="block font-medium">Preço</label>
            <input v-model.number="produtoSelecionado.price" type="number" step="0.01" class="w-full border rounded p-2" required />
          </div>
          <div class="mb-4">
            ...
            <div v-if="produtoSelecionado.image_url" class="mt-2">
              <img :src="imagemUrl(produtoSelecionado.image_url)" alt="Imagem atual" class="h-24 object-cover border rounded" />
            </div>
          </div>
          <div class="flex justify-end gap-2">
            <button type="button" @click="fecharModal" class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">Cancelar</button>
    ...
    <!-- Modal de Imagem -->
    <div v-if="modalImagemAberto" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white p-4 rounded-lg shadow-lg max-w-xl w-full relative">
        <button @click="fecharModalImagem" class="absolute top-2 right-2 text-gray-600 hover:text-black text-xl font-bold">&times;</button>
        <h3 class="text-lg font-semibold mb-4">Imagem do Produto</h3>
        <img :src="imagemUrl(produtoImagem?.imageUrl)" alt="Imagem do Produto" class="max-h-[400px] mx-auto object-contain" />
      </div>
    </div>
  </div>
</template>
...
function fecharModal() {
  ...
  imagemSelecionada.value = null
  produtoSelecionado.value = { id: null, name: '', description: '', price: 0, image_url: null }
}
